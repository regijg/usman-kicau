...
export default function BurungForm({ burung }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      let gambar_url = burung?.gambar_url ?? null

      if (file) {
        const ext = file.name.split('.').pop()
        const filename = `burung_${Date.now()}.${ext}`
      ...
      <div className="space-y-5">
        ...
        <div>
          <label className="block text-sm font-semibold text-stone-700 mb-1.5">
            Nama Burung <span className="text-red-400">*</span>
          </label>
          <input
            type="text"
            value={nama}
        ...
        <div>
          ...
          <select
            ...
            className="w-full border border-stone-200 rounded-xl px-4 py-3 text-stone-800 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition-all bg-white"
          >
            {KATEGORI.map((k) => (
